...
import hiof.magnus.TweenAccessors.SpriteAccessor;
...
public class SplashScreen implements Screen {
    ...
    public SplashScreen(MPGame game) {
        this.game = game;
    }
    ...
    private void setupTween() {
        ...
        TweenCallback cb = new TweenCallback() {
            ...
            public void onEvent(int type, BaseTween<?> source) {
                if (AssetLoader.getGameChosen() == 1) {
                    game.setScreen(new GameScreen(game));
                } else if (AssetLoader.getGameChosen() == 2) {
                    //game.setScreen(new GameScreenIG(game));
                } else {
        ...
        Tween.to(sprite, SpriteAccessor.ALPHA, .8f).target(1)
                .ease(TweenEquations.easeInOutQuad).repeatYoyo(1, .4f).setCallback(cb)
